    ...
    const defaultPins = [
        {
            ...
            icon: '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="#ffffff" stroke="#ffffff" stroke-width="0"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path fill="none" d="M0 0h24v24H0z"></path> <path d="M7.172 2.757L10.414 6h3.171l3.243-3.242a1 1 0 0 1 1.415 1.415l-1.829 1.827L18.5 6A3.5 3.5 0 0 1 22 9.5v8a3.5 3.5 0 0 1-3.5 3.5h-13A3.5 3.5 0 0 1 2 17.5v-8A3.5 3.5 0 0 1 5.5 6h2.085L5.757 4.171a1 1 0 0 1 1.415-1.415zM18.5 8h-13a1.5 1.5 0 0 0-1.493 1.356L4 9.5v8a1.5 1.5 0 0 0 1.356 1.493L5.5 19h13a1.5 1.5 0 0 0 1.493-1.356L20 17.5v-8A1.5 1.5 0 0 0 18.5 8zM8 11a1 1 0 0 1 1 1v2a1 1 0 0 1-2 0v-2a1 1 0 0 1 1-1zm8 0a1 1 0 0 1 1 1v2a1 1 0 0 1-2 0v-2a1 1 0 0 1 1-1z"></path> </g> </g></svg>',
            isDefault: true
        },
        {
            id: 'pin-2',
...
function renderPins() {
    ...
    pins.forEach((pin) => {
        ...
        if (pin.iconType === 'image') {
            ...
            img.src = pin.icon.startsWith('./')
                ? chrome.runtime.getURL(pin.icon.substring(2))
                : pin.icon
            img.alt = pin.name
            img.style.width = '100%'
            ...
            link.innerHTML = pin.icon || ''
        }

        pinContainer.appendChild(link)
    })
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedBgImage) {
        if (savedBgImage.startsWith('data:image')) {
            ...
            bgElement.src = chrome.runtime.getURL(`images/${savedBgImage}`)
        }
    } else {
        bgElement.src = chrome.runtime.getURL('images/qiyue.png')
        try {
    ...
    // Listen for pin update event
    window.addEventListener('pinsUpdated', () => {
        renderPins()
    })
})
/**
...
const linkProviders = [
    ...
    },
    {
        id: 'github',
        name: 'GitHub',
        match: (search) => search.startsWith('gh '),
    ...
    },
    {
        id: 'usyd student',
        name: 'Usyd student',
        match: (search) => search.startsWith('stu '),
        link: (search) =>
    ...
    },
    {
        id: 'usyd service',
        name: 'Usyd service',
        match: (search) => search.startsWith('ser '),
        link: (search) => `https://myuni.sydney.edu.au`,
        icon: () =>
            `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M21 10L12 5L3 10L6 11.6667M21 10L18 11.6667M21 10V10C21.6129 10.3064 22 10.9328 22 11.618V16.9998M6 11.6667L12 15L18 11.6667M6 11.6667V17.6667L12 21L18 17.6667L18 11.6667" stroke="currentcolor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>`
    },
    {
        id: 'usyd simple extension',
        name: 'Usyd simple extension',
        match: (search) => search.startsWith('spl '),
        link: (search) => `https://sydneyuni.service-now.com/sm`,
        icon: () =>
            `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M21 10L12 5L3 10L6 11.6667M21 10L18 11.6667M21 10V10C21.6129 10.3064 22 10.9328 22 11.618V16.9998M6 11.6667L12 15L18 11.6667M6 11.6667V17.6667L12 21L18 17.6667L18 11.6667" stroke="currentcolor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>`
    },
    {
    ...
    },
    {
        id: 'youtube',
        name: 'YouTube',
        match: (search) => search.startsWith('yt '),
        link: (search) =>
    ...
    },
    {
        id: 'ask',
        name: 'Perplexity',
        match: (search) => search.startsWith('ask '),
        link: (search) =>
    ...
    },
    {
        id: 'chatgpt',
        name: 'ChatGPT',
        match: (search) => search.startsWith('gpt '),
    ...
    },
    {
        id: 'claude',
        name: 'Claude',
        match: (search) => search.startsWith('cl '),
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const selectedDiv = engineSelect.querySelector('.selected')
    const optionsDivs = engineSelect.querySelectorAll('.options div')

    function capitalize(str) {
        return str.charAt(0).toUpperCase() + str.slice(1)
    }

    selectedDiv.textContent = capitalize(currentEngine)
    optionsDivs.forEach((div) => {
        if (div.dataset.engine === currentEngine) {
    ...
    optionsDivs.forEach((div) => {
        div.addEventListener('click', (e) => {
            e.stopPropagation()
            const engine = div.dataset.engine
            if (engine === currentEngine) return

            currentEngine = engine
            localStorage.setItem('searchEngine', currentEngine)
            selectedDiv.textContent = capitalize(engine)

            optionsDivs.forEach((d) => d.classList.remove('active'))
            div.classList.add('active')

            engineSelect.classList.remove('active')
        })
    })

    selectedDiv.addEventListener('click', (e) => {
        e.stopPropagation()
        engineSelect.classList.toggle('active')
    })
})
...
const detectSearchMode = (currentMode, search) => {
    ...
    const newProvider = linkProviders.find((p) => p.match(search))
    return newProvider ? newProvider.id : null
}

const getModeById = (id) => linkProviders.find((p) => p.id === id)
...
const composeLink = (search) => {
    ...
    switch (currentEngine) {
        ...
        case 'sogou':
            return `https://www.sogou.com/web?query=${encodeURIComponent(
                search
            )}`
        case 'bing':
        default:
...
document.addEventListener(
    ...
    async () => {
        ...
        const startClock = () => {
            ...
            if (parseInt(displayedMinute.textContent) !== minute) {
                displayedMinute.textContent =
                    minute <= 9 ? `0${minute}` : minute
            }

            if (parseInt(displayedHour.textContent) !== hour) {
        ...
        searchBox.addEventListener('input', () => {
            ...
            if (newMode != null) {
                mode = newMode
                modeIcon.innerHTML = linkProviders
                    .find((p) => p.id === mode)
                    ?.icon(searchBox.value)
